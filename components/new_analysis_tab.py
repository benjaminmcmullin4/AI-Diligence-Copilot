"""New Analysis tab -- Input form for submitting a diligence request."""
...
from config import DEFAULT_MODEL, get_api_key, DB_PATH, COLORS
from db import save_analysis
from schema import AnalysisRequest, BusinessModelType


def render_new_analysis() -> None:
    """Render the New Analysis input form."""

    st.markdown(
        f'<div class="section-header">New Analysis</div>',
    ...
    business_model = (
        BusinessModelType.SAAS if model_choice == "SaaS" else BusinessModelType.NON_SAAS
    )

    # ── Company Basics ──
    st.markdown(
        f'<div class="section-header">Company Information</div>',
        unsafe_allow_html=True,
    )
    ...
    # ── Financial Inputs ──
    st.markdown(
        f'<div class="section-header">Financial Data</div>',
        unsafe_allow_html=True,
    )
    ...
    if business_model == BusinessModelType.SAAS:
        ...
        with fc1:
            ...
            nrr = st.number_input(
                "Net Revenue Retention (%)", min_value=0.0, max_value=300.0, value=None, step=0.1
            )
            grr = st.number_input(
                "Gross Revenue Retention (%)", min_value=0.0, max_value=100.0, value=None, step=0.1
            )
        with fc2:
            customer_count = st.number_input("Total Customers", min_value=0, value=None, step=1)
            acv = st.number_input(
                "Average Contract Value ($K)", min_value=0.0, value=None, step=0.1
            )
            yoy_growth = st.number_input(
                "YoY Growth (%)", min_value=-100.0, value=None, step=0.1
            )
        with fc3:
            gross_margin = st.number_input(
                "Gross Margin (%)", min_value=0.0, max_value=100.0, value=None, step=0.1
            )
            cac_payback = st.number_input(
                "CAC Payback (months)", min_value=0.0, value=None, step=0.1
            )
        ...
        with fc1:
            revenue = st.number_input(
                "Revenue ($M)", min_value=0.0, value=None, step=0.1, format="%.1f"
            )
            ebitda = st.number_input(
                "EBITDA ($M)", min_value=-1000.0, value=None, step=0.1, format="%.1f"
            )
        with fc2:
            ebitda_margin = st.number_input(
                "EBITDA Margin (%)", min_value=-100.0, max_value=100.0, value=None, step=0.1
            )
            fcf = st.number_input(
                "Free Cash Flow ($M)", min_value=-1000.0, value=None, step=0.1, format="%.1f"
            )
        with fc3:
            unit_count = st.number_input(
                "Number of Units/Locations", min_value=0, value=None, step=1
            )
            same_store_growth = st.number_input(
                "Same-Store Growth (%)", min_value=-100.0, value=None, step=0.1
            )
    ...
    # ── Qualitative Inputs ──
    st.markdown(
        f'<div class="section-header">Qualitative Information</div>',
        unsafe_allow_html=True,
    )
    ...
    # ── File Upload ──
    st.markdown(
        f'<div class="section-header">Document Upload</div>',
        unsafe_allow_html=True,
    )
    ...
    if st.button("Generate Diligence", type="primary", use_container_width=True):
        ...
        if not api_key:
            st.error(
                ...
                "Add `ANTHROPIC_API_KEY` to your `.env` file or Streamlit secrets."
            )
            st.stop()
        ...
        with st.status("Running diligence pipeline...", expanded=True) as status_display:
            try:
                ...
                st.error("Pipeline module not found. Ensure `pipeline/__init__.py` exports `run_pipeline`.")
                save_analysis(analysis_id, {
                    "status": "failed",
                    "error_message": "Pipeline module not found",
                })
                st.stop()
            ...
            try:
                ...
                if result["status"] == "completed":
                    status_display.update(
                        label="Diligence complete!", state="complete", expanded=False
                    )
